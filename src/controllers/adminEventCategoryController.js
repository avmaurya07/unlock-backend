...
exports.createCategory = async (req, res) => {
  try {
    ...
    return res.status(201).json({ success: true, message: "Event category created", category: cat });
  } catch (err) {
...
exports.updateCategory = async (req, res) => {
  try {
    ...
    if (name) {
      ...
      const conflict = await EventCategory.findOne({
        _id: { $ne: id },
        $or: [
          { slug: updates.slug },
          { name: { $regex: new RegExp(`^${escapeRegex(name)}$`, "i") } },
    ...
    const updated = await EventCategory.findByIdAndUpdate(id, updates, { new: true });
    if (!updated) return res.status(404).json({ success: false, message: "Category not found" });

    return res.json({ success: true, message: "Event category updated", category: updated });
  } catch (err) {
    return res.status(500).json({ success: false, message: err.message });
  }
};
...
exports.deleteCategory = async (req, res) => {
  try {
    ...
    return res.json({ success: true, message: "Event category deleted" });
  } catch (err) {
    return res.status(500).json({ success: false, message: err.message });
  }
};
...
exports.getCategoriesAdmin = async (req, res) => {
  try {
    ...
    const categories = await EventCategory.find(filter).sort({ createdAt: -1 });
